...
from dataset import collate_fn_captions, collate_fn_classifier, collate_fn_improved
...
def improved_eval_captions(dataset, model, filename):
    '''
    Function that tests a model
    ...
    Return:
        None
    '''
    testloader = DataLoader(dataset, batch_size=1, shuffle=False, collate_fn=partial(collate_fn_improved, word2idx=dataset.word2idx, training=True))
    # Set the correct device
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    model = model.to(device)
    # Create the conversion id -> token
